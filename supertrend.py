...
def signal_handler(signum, frame):
    global running
    print("Signal received, stopping the algorithm...")
    logger.info("Signal received, stopping the algorithm...")
    running = False
...
def main():
    print("Supertrend algorithm started...")
    logger.info("Supertrend algorithm started...")
    while running:
        ...
        def get_time_series(exchange, token, days, interval):
            ...
            if ret:
                ...
                logger.info("No Data for the given exchange, token, days and interval")
        ...
        def get_mtm():
                        ...
                        try:
                            ...
                            # logger.info('no open positions for the day, waiting for 1 minute before checking again')
            ...
            if current_time3 < "15:25" and -Mtm_SL <= mtm <= Mtm_target:
                ...
                if SIGNAL and not position_open:
                    ...
                    if entry_order['stat'] == 'Ok':
                        ...
                        logger.info(f"Order placed successfully: {ordno}")

                    else:
                        print(f"Order error: {entry_order['emsg']}")
                        logger.info(f"Order error: {entry_order['emsg']}")
                    ...
                    while position_open:
                        ...
                        if exit_status == "COMPLETE":
                            ...
                            logger.info(f"Exit order {exit_ordno} completed. Re-entering after 1 minute.")
                            ...
                            logger.info("Exit order placed but not completed. Retrying in 5 seconds.")
                            time.sleep(3)
            
                else:
                    print("No valid trading signal or position already open.")
                    logger.info("No valid trading signal or position already open.")
                    time.sleep(22)  # Short pause before re-evaluating signals
            else:
                print('you cant take position after 3:20')
                logger.info("you cant take position after 3:20")
                time.sleep(30)
        ...
        print("Running supertrend algorithm...")
        logger.info("Running supertrend algorithm...")
        time.sleep(10)  # Simulating work (replace this with your logic)
    print("Supertrend algorithm stopped.")
    logger.info("Supertrend algorithm stopped.")
